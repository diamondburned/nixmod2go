...
Running `nixmod2go -f go module.nix` will output the following Go code:
...
// Services is the struct type for `config.services`.
type Services struct {
	XYZ XYZ `json:"xyz"`
}

// XYZ is the struct type for `config.services.xyz`.
type XYZ struct {
